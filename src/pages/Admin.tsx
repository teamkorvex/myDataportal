...
export function Admin() {
  ...
  const handleUpdateUser = () => {
    ...
    if (editIsSuspended !== selectedUser.isSuspended) {
      updates.isSuspended = editIsSuspended;
    }

    if (Object.keys(updates).length > 0) {
      const success = updateUserByAdmin(selectedUser.id, updates);
      if (success) {
  ...
  const handleTogglePremium = (userData: User) => {
    const success = updateUserByAdmin(userData.id, { isPremium: !userData.isPremium });
    if (success) {
      loadUsers();
    }
  };

  const handleToggleSuspend = (userData: User) => {
    const success = updateUserByAdmin(userData.id, { isSuspended: !userData.isSuspended });
    if (success) {
      loadUsers();
    }
  };
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">
                    {allUsers.filter(u => u.isSuspended).length}
                  </p>
                </div>
              </div>
            </CardContent>
                ...
                >
                  ...
                  <div className="col-span-2 flex justify-end gap-1">
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className={`h-8 w-8 ${userData.isPremium ? 'text-yellow-500 hover:text-yellow-600' : 'text-muted-foreground hover:text-yellow-500'}`}
                      onClick={() => handleTogglePremium(userData)}
                      title={userData.isPremium ? 'Remove Premium' : 'Give Premium'}
                    >
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className={`h-8 w-8 ${userData.isSuspended ? 'text-green-500 hover:text-green-600' : 'text-muted-foreground hover:text-destructive'}`}
                      onClick={() => handleToggleSuspend(userData)}
                      title={userData.isSuspended ? 'Unsuspend' : 'Suspend'}
                    >
                      {userData.isSuspended ? <Check className="w-4 h-4" /> : <Ban className="w-4 h-4" />}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
        ...
        <DialogContent className="sm:max-w-md bg-card border-border">
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="flex items-center justify-between p-3 bg-secondary/50 rounded">
              ...
              <Button
                variant={editIsPremium ? 'default' : 'outline'}
                size="sm"
                className={editIsPremium ? 'bg-yellow-500 hover:bg-yellow-600' : ''}
                onClick={() => setEditIsPremium(!editIsPremium)}
              >
                {editIsPremium ? 'Premium' : 'Standard'}
              </Button>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-secondary/50 rounded">
              ...
              <Button
                variant={editIsSuspended ? 'destructive' : 'outline'}
                size="sm"
                onClick={() => setEditIsSuspended(!editIsSuspended)}
              >
                {editIsSuspended ? 'Suspended' : 'Active'}
              </Button>
            </div>
